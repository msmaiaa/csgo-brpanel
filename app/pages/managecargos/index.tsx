...
const ManageCargos = ({ user }: Props) => {
    ...
    if (checkbox) {
      ...
      if (isSingleServer) {
        ...
        if (e.target.checked) {
          ...
          if (!updateInputs[cargo.name].individual)
            updateInputs[cargo.name].individual = true;
        } else {
          newState[cargo.name].servers = newState[cargo.name].servers.filter(
            (sv) => {
  ...
  const handleAddCargo = async () => {
    try {
      ...
      let serversData = serversChecked.filter((sv) => {
        return sv.checked;
      });
      if (isAllServers) {
        serversData = servers;
          ...
          <div className={styles.cardWrapper}>
            ...
            >
              {isLoadingCargos ? (
                ...
                <>
                  {cargosFromDb.length > 0 ? (
                    cargosFromDb.map((cargo) => {
                      if (updateInputs[cargo.name])
                        return (
                          <Accordion
                            key={cargo.id}
                            ...
                            >
                              ...
                              >
                                <TextField
                                  ...
                                  inputProps={{ maxLength: 100 }}
                                  name="name"
                                  value={updateInputs[cargo.name].name}
                                  onChange={(event) =>
                                    handleUpdateChange(event, cargo)
                                  }
                                  required
                                  label="Nome do cargo"
                                />
                                <TextField
                                  ...
                                  inputProps={{ maxLength: 100 }}
                                  name="price"
                                  value={updateInputs[cargo.name].price}
                                  onChange={(event) =>
                                    handleUpdateChange(event, cargo)
                                  }
                                  required
                                ...
                                <TextField
                                  ...
                                  inputProps={{ maxLength: 100 }}
                                  name="duration"
                                  value={updateInputs[cargo.name].duration}
                                  onChange={(event) =>
                                    handleUpdateChange(event, cargo)
                                  }
                                  required
                                ...
                                <TextField
                                  ...
                                  inputProps={{ maxLength: 100 }}
                                  name="flags"
                                  value={updateInputs[cargo.name].flags}
                                  onChange={(event) =>
                                    handleUpdateChange(event, cargo)
                                  }
                                  required
                                  ...
                                  <>
                                    <div className={styles.serverField}>
                                      <Checkbox
                                        ...
                                        name="individual"
                                        checked={
                                          !updateInputs[cargo.name].individual
                                        }
                                        onChange={(event) =>
                                          handleUpdateChange(event, cargo, true)
                                        }
                                        style={{ color: theme.data.textAccent }}
                                      />
                                      <p>Todos os servidores</p>
