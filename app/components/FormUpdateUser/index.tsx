...
const FormUpdateUser = ({ selectedData, updateUserInfo }: Props) => {
  ...
  const handleUpdateUType = async () => {
    try {
      const updatedUser = await updateUser(selectedData.id, {
  ...
  const handleAddCargo = async (
    ...
    try {
      const addedCargos = await addCargosToUser({
        cargo,
        days,
        server,
        user: selectedData,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column" }}>
      ...
      <div
        style={{ marginLeft: "20px", marginTop: "35px", marginRight: "20px" }}
      >
        <p className={styles.title}>Adicionar cargos</p>
        <p style={{ color: "gray", marginTop: "5px", fontSize: "14px" }}>
          Caso o usuário já possua um cargo no servidor, as flags do novo cago
          irão sobrepor às do cargo antigo e os dias serão somados.
        </p>
                ...
                {cargosAllServers.map((cargo: ICargo) => {
                  return (
                    ...
                    >
                      ...
                      <>
                        <Button
                          onClick={() => handleAddCargo(cargo, 1, "all")}
                          variant="contained"
                          color="primary"
                        ...
                        <Button
                          onClick={() => handleAddCargo(cargo, 15, "all")}
                          variant="contained"
                          color="secondary"
                          className={styles.button}
                        >
                        ...
                        <Button
                          onClick={() => handleAddCargo(cargo, 30, "all")}
                          variant="contained"
                  ...
                  <Accordion key={server.id} className={classes.accordion}>
                    ...
                    >
                      {server.cargo_server.length > 0 && (
                        <>
                          {server.cargo_server.map(
                            (cargo_sv: ICargo_Server) => {
                              return (
                                <div
                                  key={cargo_sv.id}
                                  style={{
                                ...
                                >
                                  ...
                                  <Button
                                    onClick={() =>
                                      handleAddCargo(cargo_sv.cargo, 1, server)
                                    }
                                    variant="contained"
                                    color="primary"
                                    className={styles.button}
                                  ...
                                  <Button
                                    onClick={() =>
                                      handleAddCargo(cargo_sv.cargo, 15, server)
                                    }
                                    variant="contained"
                                    color="secondary"
                                    className={styles.button}
                                  ...
                                  <Button
                                    onClick={() =>
                                      handleAddCargo(cargo_sv.cargo, 30, server)
                                    }
                                    variant="contained"
                                    style={{
                                      backgroundColor: "red",
