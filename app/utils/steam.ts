...
export async function fetchProfile(data){
  ...
  if(!parsedSteam) {
    throw new Error('Não foi possível encontrar um usuário com os dados inseridos.')
  }
...
const getSteamID64FromString = async(data) => {
  ...
  const regexList = [
    ...
    {
      name: 'steamUrl',
      regex: new RegExp('(?:https?:\\/\\/)?steamcommunity\\.com\\/(?:profiles)\\/[a-zA-Z0-9]+')
    },
    {
      name: 'steamCustomUrl',
      regex: new RegExp('(?:https?:\\/\\/)?steamcommunity\\.com\\/(?:id)\\/[a-zA-Z0-9]+')
    },
  ]
  for(let reg of regexList) {
    if(reg.regex.test(data)) {
      if(reg.name === 'steamid3' || reg.name === 'steamid') {
        ...
        returnData.steamid = new SteamID.ID(vanityResult.data.response.steamid).getSteamID2()
      }
    }
  }
  return returnData
